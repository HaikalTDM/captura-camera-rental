...
export default function MobileBookingDetail() {
  ...
  const handleCompleteAll = async () => {
    ...
    const toastId = 'complete-all-toast'; // We'll implement a simple toast or use existng message system

    try {
      showSuccessMessage('Processing full completion...');
      ...
      await loadBooking();
      showSuccessMessage('✨ Booking fully completed!');

    } catch (error) {
      console.error('Error completing all:', error);
      showSuccessMessage('❌ Error processing completion');
    } finally {
      setIsUpdating(false);
    }
  };
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      if (showPickupModal || showReturnModal || showDeleteConfirm || showCompleteAllConfirm) {
        document.body.style.overflow = 'hidden';
      } else {
        document.body.style.overflow = 'auto';
      }
    ...
    }, [showPickupModal, showReturnModal, showDeleteConfirm, showCompleteAllConfirm]);
    ...
    return (
      <div className={`min-h-screen ${isDarkMode ? 'bg-slate-950' : 'bg-slate-50'} pb-24`}>
          ...
          <div className={`relative overflow-hidden rounded-3xl ${isDarkMode ? 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900' : 'bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900'} p-5 sm:p-6 shadow-2xl border ${isDarkMode ? 'border-slate-700' : 'border-slate-700'}`}>
            ...
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-2 mb-4">
                <span className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold ${booking.booking_status === 'confirmed' || booking.booking_status === 'approved'
                  ...
                  <div className={`w-1.5 h-1.5 rounded-full ${booking.booking_status === 'confirmed' || booking.booking_status === 'approved'
                    ? 'bg-emerald-400'
                    : booking.booking_status === 'pending_approval'
                      ? 'bg-amber-400'
                      : booking.booking_status === 'completed'
              ...
              {(() => {
                ...
                return (
                  <button
                    ...
                    className={`mt-4 p-4 sm:p-5 rounded-2xl border backdrop-blur-sm flex items-center gap-3 w-full transition-all duration-200 ${isComplete
                  ...
                  >
                    <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center flex-shrink-0 ${isComplete ? 'bg-green-500/20' : 'bg-blue-500/20'
                      }`}>
                      {getActionIcon()}
                    </div>
                    <div className="flex-1 min-w-0 text-left">
                      <p className={`text-[10px] sm:text-xs font-semibold uppercase tracking-wide ${isComplete ? 'text-green-400' : 'text-blue-400'
                        }`}>
        ...
        {/* Quick Actions - Complete All Button */}
        {booking.booking_status !== 'completed' && (
          <button
            onClick={() => setShowCompleteAllConfirm(true)}
            disabled={isUpdating}
            className={`w-full p-4 rounded-2xl border flex items-center gap-3 transition-all duration-200 active:scale-[0.98] shadow-lg ${isDarkMode
              ? 'bg-gradient-to-r from-emerald-900/40 to-emerald-800/40 border-emerald-500/30 hover:bg-emerald-800/40'
              : 'bg-gradient-to-r from-emerald-50 to-emerald-100/50 border-emerald-200 hover:bg-emerald-100'
              }`}
          >
            ...
            <div className="text-left flex-1">
              <p className={`text-sm font-bold ${isDarkMode ? 'text-emerald-100' : 'text-emerald-900'}`}>
                Complete All
              </p>
              <p className={`text-xs ${isDarkMode ? 'text-emerald-400' : 'text-emerald-700'} font-medium`}>
                Mark everything as done
              </p>
            </div>
            <svg className={`w-5 h-5 ${isDarkMode ? 'text-emerald-500' : 'text-emerald-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
            </svg>
          </button>
        )}
        ...
        <div className={`${isDarkMode ? 'bg-slate-900/50 border-slate-800' : 'bg-white border-slate-200'} border rounded-3xl p-5 sm:p-6 shadow-lg backdrop-blur-sm`}>
          ...
          <div className="space-y-3">
            ...
            <div className={`rounded-2xl p-4 sm:p-5 border-2 transition-all duration-300 ${booking.equipment_picked_up
              ...
              <div className="flex items-start justify-between mb-4 gap-3">
                <div className="flex items-center gap-2.5 sm:gap-3 flex-1 min-w-0">
                  <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 flex-shrink-0 ${booking.equipment_picked_up
                ...
                <span className={`px-2.5 sm:px-3 py-1.5 rounded-xl text-[10px] sm:text-xs font-bold shadow-sm flex-shrink-0 ${booking.equipment_picked_up
              ...
              {booking.equipment_condition_pickup && (
                <div className="mb-3">
                  <span className={`inline-block px-3 py-1 rounded-lg text-xs font-bold ${booking.equipment_condition_pickup === 'excellent' ? 'bg-emerald-100 text-emerald-800' :
              ...
              <button
                ...
                disabled={isUpdating}
                className={`w-full py-3.5 sm:py-4 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 active:scale-[0.98] shadow-lg min-h-[48px] ${booking.equipment_picked_up
            ...
            <div className={`rounded-2xl p-4 sm:p-5 border-2 transition-all duration-300 ${booking.equipment_returned
              ...
              <div className="flex items-start justify-between mb-4 gap-3">
                <div className="flex items-center gap-2.5 sm:gap-3 flex-1 min-w-0">
                  <div className={`w-11 h-11 sm:w-12 sm:h-12 rounded-xl flex items-center justify-center shadow-lg transition-all duration-300 flex-shrink-0 ${booking.equipment_returned
                ...
                <span className={`px-2.5 sm:px-3 py-1.5 rounded-xl text-[10px] sm:text-xs font-bold shadow-sm flex-shrink-0 ${booking.equipment_returned
              ...
              {booking.equipment_condition_return && (
                <div className="mb-3">
                  <span className={`inline-block px-3 py-1 rounded-lg text-xs font-bold ${booking.equipment_condition_return === 'excellent' ? 'bg-emerald-100 text-emerald-800' :
              ...
              <button
                ...
                className={`w-full py-3.5 sm:py-4 rounded-xl text-xs sm:text-sm font-bold transition-all duration-200 active:scale-[0.98] shadow-lg min-h-[48px] ${booking.equipment_returned
                  ? 'bg-amber-500 hover:bg-amber-600 text-white shadow-amber-500/30'
                  : booking.equipment_picked_up
            ...
            <div className="px-6 pb-6 space-y-3 animate-fadeIn border-t border-slate-200 dark:border-slate-800 pt-5">
              ...
              <div className={`rounded-2xl p-4 border-2 transition-all ${booking.deposit_paid ? 'bg-emerald-50 border-emerald-200' : isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-3 py-1.5 rounded-xl text-xs font-bold ${booking.deposit_paid ? 'bg-emerald-500 text-white' : 'bg-red-100 text-red-700'
                    }`}>
                ...
                <button
                  ...
                  disabled={isUpdating}
                  className={`w-full py-3 rounded-xl text-sm font-bold transition-all duration-200 active:scale-[0.98] shadow-lg ${booking.deposit_paid
              ...
              <div className={`rounded-2xl p-4 border-2 transition-all ${booking.final_payment_paid ? 'bg-emerald-50 border-emerald-200' : isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex items-center justify-between mb-3">
                  ...
                  <span className={`px-3 py-1.5 rounded-xl text-xs font-bold ${booking.final_payment_paid ? 'bg-emerald-500 text-white' : 'bg-red-100 text-red-700'
                    }`}>
                ...
                <button
                  ...
                  disabled={isUpdating}
                  className={`w-full py-3 rounded-xl text-sm font-bold transition-all duration-200 active:scale-[0.98] shadow-lg ${booking.final_payment_paid
                    ? 'bg-amber-500 text-white shadow-amber-500/30'
                    : 'bg-emerald-500 text-white shadow-emerald-500/30'
                    } disabled:opacity-50`}
                >
              ...
              {booking.deposit_paid && (
                <div className={`rounded-2xl p-4 border-2 transition-all ${booking.deposit_refunded ? 'bg-purple-50 border-purple-200' : isDarkMode ? 'bg-slate-800/50 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`px-3 py-1.5 rounded-xl text-xs font-bold ${booking.deposit_refunded ? 'bg-purple-500 text-white' : 'bg-amber-100 text-amber-700'
                      }`}>
                  ...
                  <button
                    ...
                    disabled={isUpdating}
                    className={`w-full py-3 rounded-xl text-sm font-bold transition-all duration-200 active:scale-[0.98] shadow-lg ${booking.deposit_refunded
                      ? 'bg-amber-500 text-white shadow-amber-500/30'
