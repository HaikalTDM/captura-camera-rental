...
export default function CustomerGallery() {
  ...
  return (
    <section className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 py-8 sm:py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        {/* Header with Personality */}
        <div className="text-center mb-10 sm:mb-16 animate-fadeIn">
          <div className="inline-flex items-center gap-2 bg-black text-white px-4 py-2 rounded-full text-xs font-black mb-4 shadow-lg">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
            </svg>
            <span>Customer Gallery</span>
          </div>
          <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-black mb-3 tracking-tight">
        ...
        {isLoading ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6">
            {[...Array(9)].map((_, i) => (
              <div 
                key={i} 
                className={`w-full aspect-square bg-slate-200 rounded-2xl animate-pulse ${getGridClass(i)}`}
              ></div>
            ))}
          </div>
          ...
          <>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 sm:gap-6 auto-rows-fr">
              {customerImages.map((image, index) => {
                ...
                return (
                  <div
                    key={image.id}
                    className={`group cursor-pointer relative overflow-hidden bg-slate-100 aspect-square rounded-2xl sm:rounded-3xl shadow-md hover:shadow-2xl transition-all duration-500 animate-scaleIn ${getGridClass(index)}`}
                    style={{ animationDelay: `${index * 50}ms` }}
                    onClick={() => setSelectedImage(image)}
                  >
                    {/* Loading Skeleton */}
                    {!loadedImages.has(image.id) && (
                      <div className="absolute inset-0 bg-slate-200 animate-pulse rounded-2xl sm:rounded-3xl"></div>
                    )}
                    ...
                    <div className="absolute top-3 right-3 sm:top-4 sm:right-4">
                      <div className="bg-white/90 backdrop-blur-md px-3 py-1.5 sm:px-4 sm:py-2 rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all duration-300 transform scale-95 group-hover:scale-100">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <div className="w-14 h-14 sm:w-16 sm:h-16 bg-white/95 backdrop-blur-md rounded-full flex items-center justify-center transform scale-75 group-hover:scale-100 transition-transform shadow-2xl">
            ...
            <div className="text-center mt-16 sm:mt-24 animate-fadeIn" style={{ animationDelay: '400ms' }}>
              <div className="max-w-2xl mx-auto bg-gradient-to-br from-black via-slate-900 to-black rounded-3xl p-8 sm:p-12 shadow-2xl border border-slate-800 relative overflow-hidden">
                ...
                <div className="relative z-10">
                  <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur-md px-4 py-2 rounded-full text-xs font-black text-white mb-6 border border-white/20">
                  ...
                  <p className="text-slate-300 text-sm sm:text-base font-semibold mb-8 max-w-lg mx-auto">
                  ...
                  <a
                    ...
                    className="inline-flex items-center gap-3 bg-white text-black font-black py-4 px-8 sm:py-5 sm:px-10 rounded-2xl hover:scale-105 transition-all duration-300 shadow-2xl active:scale-95 text-sm sm:text-base group"
                  >
                  ...
                  <p className="text-slate-400 text-xs sm:text-sm font-semibold mt-6">
      ...
      {selectedImage && (
        <div
          className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center p-4 animate-fadeIn cursor-zoom-out backdrop-blur-sm"
          onClick={() => setSelectedImage(null)}
        >
          <button
            className="absolute top-4 right-4 sm:top-6 sm:right-6 w-12 h-12 bg-white/10 backdrop-blur-md rounded-full flex items-center justify-center text-white hover:bg-white/20 transition-all z-10 border border-white/20"
            onClick={() => setSelectedImage(null)}
          >
          ...
          <div className="max-w-6xl w-full max-h-[90vh] animate-scaleIn cursor-default" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-8 text-center text-white animate-slideUp">
              <div className="inline-flex items-center gap-3 bg-white/10 backdrop-blur-md px-6 py-3 rounded-full mb-3 border border-white/20">
              ...
              {selectedImage.location && (
                <div className="text-sm text-slate-300 font-semibold flex items-center justify-center gap-2">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" />
                  </svg>
                  {selectedImage.location}
