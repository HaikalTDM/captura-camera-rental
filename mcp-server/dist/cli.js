 ...
 */
import { listCameras, getCamera, checkAvailability, } from './tools/cameras.tools.js';
import { listBookings, getBooking, searchBookings, getTodayReturns, smartCreateBooking, bulkApproveBookings, getOverduePayments, getNextActions, } from './tools/bookings.tools.js';
import { listCustomers, getCustomer, } from './tools/customers.tools.js';
import { getDashboardSummary, } from './tools/admin.tools.js';
...
function printCompact(label, value) {
...
async function fetchApi(path, method, body) {
    ...
    if (MIRROR_SECRET) {
        init.headers['X-Captura-Mirror-Secret'] = MIRROR_SECRET;
    }
...
async function main() {
    ...
    const cmd = (parsed._cmd || '').toLowerCase();
    const subcmd = (parsed._args?.[0] || '').toLowerCase();
    const posArgs = parsed._args || [];
    ...
    try {
        switch (cmd) {
            ...
            case 'cameras': {
                switch (subcmd) {
                    case 'list': {
                        const filter = parsed.filter === 'all' ? 'all' : 'available_only';
                        const sortBy = parsed.sortBy || 'display_order';
                        const cameras = await listCameras(filter, sortBy);
                        if (jsonMode)
                            print(cameras);
                        else
                    ...
                    case 'get': {
                        ...
                        if (!cameraId) {
                            process.stderr.write('Usage: cameras get CAMERA_ID\n');
                            process.exit(1);
                        }
                    ...
                    case 'availability': {
                        const camId = posArgs[1];
                        const start = posArgs[2];
                        const end = posArgs[3];
                        if (!camId || !start || !end) {
                            process.stderr.write('Usage: cameras availability CAMERA_ID START END\n');
                            process.exit(1);
                        }
            ...
            case 'bookings': {
                switch (subcmd) {
                    case 'list': {
                        ...
                        if (jsonMode) {
                            ...
                            for (const b of bookings) {
                                const cust = b.customer?.full_name || 'N/A';
                                const cam = b.camera?.name || 'N/A';
                    ...
                    case 'get': {
                        ...
                        if (!bookingId) {
                            process.stderr.write('Usage: bookings get BOOKING_ID\n');
                            process.exit(1);
                        }
                    ...
                    case 'search': {
                        const query = posArgs[1] || parsed.search;
                        if (!query) {
                            process.stderr.write('Usage: bookings search QUERY\n');
                            process.exit(1);
                        }
                    ...
                    case 'approve': {
                        const bookingId = posArgs[1];
                        if (!bookingId) {
                            process.stderr.write('Usage: bookings approve BOOKING_ID [--notes NOTES]\n');
                            process.exit(1);
                        }
                    ...
                    case 'reject': {
                        ...
                        if (!bookingId || !reason) {
                            process.stderr.write('Usage: bookings reject BOOKING_ID --reason REASON\n');
                            process.exit(1);
                        }
                    ...
                    case 'cancel': {
                        const bookingId = posArgs[1];
                        if (!bookingId) {
                            process.stderr.write('Usage: bookings cancel BOOKING_ID [--reason REASON]\n');
                            process.exit(1);
                        }
                    ...
                    case 'pickup': {
                        const bookingId = posArgs[1];
                        if (!bookingId) {
                            process.stderr.write('Usage: bookings pickup BOOKING_ID [--condition CONDITION]\n');
                            process.exit(1);
                        }
                    ...
                    case 'return': {
                        const bookingId = posArgs[1];
                        if (!bookingId) {
                            process.stderr.write('Usage: bookings return BOOKING_ID [--condition CONDITION]\n');
                            process.exit(1);
                        }
                    ...
                    case 'smart-create': {
                        const fields = {
                            camera_query: parsed.cameraQuery,
                            customer_name: parsed.customerName,
                            customer_email: parsed.customerEmail,
                            customer_phone: parsed.customerPhone,
                            start_date: parsed.startDate,
                    ...
                    case 'bulk-approve': {
                        const idsStr = parsed.bookingIds;
                        if (!idsStr) {
                            process.stderr.write('Usage: bookings bulk-approve --booking-ids ID1,ID2,ID3\n');
                            process.exit(1);
                        }
                        const bookingIds = idsStr.split(',').map(s => s.trim()).filter(Boolean);
                        const result = await bulkApproveBookings(bookingIds, parsed.notes);
                        print(result);
                        break;
                    }
                    case 'next-actions': {
                        ...
                        if (jsonMode)
                            ...
                            const fmt = (arr) => arr.map(b => `${b.id} | ${b.customer?.full_name || 'N/A'} | ${b.camera?.name || 'N/A'} | ${b.start_date || '-'} → ${b.end_date || '-'}`);
            ...
            case 'customers': {
                switch (subcmd) {
                    ...
                    case 'get': {
                        ...
                        if (!customerId) {
                            process.stderr.write('Usage: customers get CUSTOMER_ID\n');
                            process.exit(1);
                        }
