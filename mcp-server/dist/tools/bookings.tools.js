...
export async function createBooking(fields) {
    ...
    const customerPhone = fields.customer_phone;
    if (!customerEmail) {
    ...
    const { data: customer } = await supabase
        ...
        .eq('id', customerId)
        .single();
    return { booking: booking, customer: (customer || { id: customerId }) };
}
export async function approveBooking(bookingId, notes) {
    const supabase = getSupabaseAdmin();
    const updates = {
...
export async function markPickup(bookingId, pickupNotes, condition) {
    const supabase = getSupabaseAdmin();
    const updates = {
...
export async function getOverduePayments(limit) {
    const supabase = getSupabaseAdmin();
    const today = new Date().toISOString().split('T')[0];
    const { data, error } = await supabase
        .from('bookings')
...
export async function completeBookingWorkflow(bookingId, options) {
    ...
    await markPickup(bookingId, options?.pickupNotes || 'Completed via MCP', options?.pickupCondition || 'good');
    // Step 2: mark returned
    await markReturn(bookingId, options?.returnNotes || 'Returned via MCP', options?.returnCondition || 'good');
...
export async function markReturn(bookingId, returnNotes, condition) {
    const supabase = getSupabaseAdmin();
    const updates = {
...
export async function smartCreateBooking(fields) {
    ...
    const matches = (cameras || []).filter((c) => c.name.toLowerCase().includes(query) ||
    ...
    if (matches.length > 1) {
        ...
        throw new BusinessRuleError(`Multiple cameras match "${fields.camera_query}": ${options.join('; ')}. Please specify which one.`);
    }
    const matchedCamera = matches[0];
    ...
    if (conflicts && conflicts.length >= (matchedCamera.available_quantity || 1)) {
        throw new ConflictError(`${matchedCamera.name} is not available for ${fields.start_date} to ${fields.end_date}. ${conflicts.length} conflicting booking(s).`);
    }
...
export async function bulkApproveBookings(bookingIds, notes) {
    const approved = [];
    const failed = [];
...
//# sourceMappingURL=bookings.tools.js.map