...
import {
  paginationSchema, dateRangeSchema, cameraIdSchema, bookingIdSchema, customerIdSchema,
  cameraCreateSchema, cameraUpdateSchema, cameraAvailabilitySchema,
  bookingCreateSchema, bookingApproveSchema, bookingRejectSchema,
  bookingCancelSchema, bookingPickupSchema, bookingReturnSchema,
  customerUpdateSchema, paymentRecordSchema, depositRefundSchema,
  invoiceGenerateSchema, settingsUpdateSchema, searchQuerySchema,
  revenueReportSchema, dashboardSummarySchema,
...
import {
  ...
  createCamera, updateCamera, setCameraAvailability,
...
import {
  ...
  markPickup, markReturn, completeBookingWorkflow, deleteBooking,
...
import {
  recordPayment, markDepositRefunded,
} from './tools/payments.tools.js';
import { generateInvoice } from './tools/invoices.tools.js';
import {
  getSettings, updateSetting,
  getDashboardSummary, getRevenueReport,
} from './tools/admin.tools.js';

function authGate(toolName: string, args: Record<string, unknown>) {
...
function stripAuthArg(args: Record<string, unknown>): Record<string, unknown> {
...
export function createServer(): McpServer {
  ...
  server.tool(
    'captura.cameras.list',
    'List all cameras in inventory. Shows pricing, availability, and discount thresholds.',
    {
  ...
  server.tool(
    'captura.cameras.get',
    'Get detailed information about a specific camera by ID.',
    {
  ...
  server.tool(
    'captura.cameras.check_availability',
    'Check if a specific camera is available for a given date range. Returns availability status and any conflicting bookings.',
    {
  ...
  server.tool(
    'captura.cameras.admin.create',
    'Add a new camera to the inventory. Requires admin authentication.',
    {
  ...
  server.tool(
    'captura.cameras.admin.update',
    'Update camera details including pricing, availability, and specifications. Requires admin authentication.',
    {
      camera_id: z.string().uuid(),
  ...
  server.tool(
    'captura.cameras.admin.set_availability',
    'Toggle camera availability on/off. Use to mark cameras as in maintenance or available for rent.',
    {
      camera_id: z.string().uuid(),
  ...
  server.tool(
    'captura.bookings.list',
    'List bookings with optional filtering by status, date range, or camera. Requires authentication to view customer details.',
    {
    ...
    async (args) => {
      try {
        ...
        const bookings = await listBookings(stripAuthArg(args) as {
          status?: string; date_from?: string; date_to?: string; camera_id?: string; limit: number; offset: number;
        });
  ...
  server.tool(
    'captura.bookings.get',
    'Get full details of a single booking including customer and camera information.',
    {
  ...
  server.tool(
    'captura.bookings.today_returns',
    'Get all bookings due for return today that have not been returned yet.',
    {
      _apiKey: z.string().optional(),
    },
    async (args) => {
  ...
  server.tool(
    'captura.bookings.admin.create',
    'Create a new booking manually. Automatically creates or updates the customer record. Requires admin authentication.',
    {
      camera_id: z.string().uuid(),
  ...
  server.tool(
    'captura.bookings.admin.approve',
  ...
  server.tool(
    'captura.bookings.admin.cancel',
    'Cancel a booking. Requires explicit confirmation via confirm=true parameter.',
    {
      booking_id: z.string().uuid(),
  ...
  server.tool(
    'captura.bookings.admin.mark_pickup',
    'Mark equipment as picked up by the customer. Updates booking status to active.',
    {
      booking_id: z.string().uuid(),
  ...
  server.tool(
    'captura.bookings.admin.mark_return',
    'Mark equipment as returned by the customer. Updates booking status to completed.',
    {
      booking_id: z.string().uuid(),
  ...
  server.tool(
    'captura.bookings.admin.complete',
    'Complete the full booking workflow in one operation: mark pickup, mark return, and refund deposit.',
    {
      booking_id: z.string().uuid(),
  ...
  server.tool(
    'captura.bookings.admin.delete',
    'Delete a booking permanently. Requires admin authentication. IRREVERSIBLE.',
    {
      booking_id: z.string().uuid(),
      confirm: z.literal(true),
      _apiKey: z.string().optional(),
  ...
  server.tool(
    'captura.bookings.overdue',
    'List bookings with overdue final payments (returned but final payment not yet settled).',
    {
  ...
  server.tool(
    'captura.bookings.next_actions',
    'Get the operational action queue: pending approvals, today\'s pickups, today\'s returns, and overdue payments.',
    {
  ...
  server.tool(
    ...
    'Create a booking with fuzzy camera name matching. Accepts "osmo", "r50", "action", "fuji" etc. Automatically checks availability and applies bulk discounts. Single call replaces 4 separate steps.',
    {
  ...
  server.tool(
    ...
    'Approve multiple pending bookings at once. Accepts up to 50 booking IDs. Returns lists of approved and failed IDs.',
    {
  ...
  server.tool(
    'captura.customers.list',
    'List customers with optional search by name, email, or phone.',
    {
  ...
  server.tool(
    'captura.customers.get',
    'Get customer details with count of their booking history.',
    {
  ...
  server.tool(
    'captura.customers.admin.update',
    'Update customer details. Requires admin authentication.',
    {
      customer_id: z.string().uuid(),
  ...
  server.tool(
    'captura.payments.admin.record',
    'Record a payment (deposit, final payment, or refund) for a booking. Requires admin authentication.',
    {
      booking_id: z.string().uuid(),
  ...
  server.tool(
    'captura.payments.admin.mark_deposit_refunded',
    'Mark the security deposit as refunded for a booking. Creates a refund payment record and updates the booking.',
    {
      booking_id: z.string().uuid(),
  ...
  server.tool(
    'captura.invoices.admin.generate',
    'Generate a PDF-ready invoice for a booking. Creates a snapshot-based invoice record. Requires admin authentication.',
    {
      booking_id: z.string().uuid(),
      _apiKey: z.string().optional(),
    },
  ...
  server.tool(
    'captura.admin.get_settings',
    'Get business settings. Returns all settings or a specific one by key.',
    {
  ...
  server.tool(
    'captura.admin.update_settings',
    'Update a business setting value. Requires admin authentication.',
    {
  ...
  server.tool(
    'captura.admin.dashboard_summary',
    'Get dashboard KPIs: active bookings, pending approvals, revenue, available cameras, new customers.',
    {
  ...
  server.tool(
    'captura.admin.revenue_report',
    'Generate a revenue report grouped by camera or month for a date range.',
    {
      start_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
      end_date: z.string().regex(/^\d{4}-\d{2}-\d{2}$/),
