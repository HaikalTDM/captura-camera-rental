-- CAPTURA Production Database Diagnosis (Fixed Version)
...
-- Check RLS status for each table (compatible version)
SELECT 
    schemaname,
    tablename,
    rowsecurity
...
-- Check if customers table has 'whatsapp' column
SELECT 
    CASE 
        WHEN EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'customers' AND column_name = 'whatsapp') 
        THEN 'EXISTS' 
        ELSE 'MISSING' 
    END as customers_whatsapp_column;
...
SELECT 'SAMPLE CAMERA DATA:' as info;
SELECT id, name, daily_rate, deposit_amount, is_available 
FROM cameras 
LIMIT 3;

-- Show sample customer data (if any exists)
SELECT 'SAMPLE CUSTOMER DATA:' as info;
SELECT id, full_name, email, phone 
FROM customers 
LIMIT 3;

-- Show sample booking data (if any exists)
SELECT 'SAMPLE BOOKING DATA:' as info;
SELECT id, camera_id, customer_id, status, total_amount 
FROM bookings 
LIMIT 3;
